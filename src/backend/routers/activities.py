...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level (e.g., 'Beginner', 'Intermediate', 'Advanced')
                  Use 'All' to show only activities without a difficulty level
    """
    ...
    if difficulty:
        if difficulty == "All":
            # Show only activities without a difficulty level
            query["difficulty"] = {"$exists": False}
        else:
            # Show activities with the specified difficulty level
